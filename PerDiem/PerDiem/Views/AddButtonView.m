...
- (IBAction)buttonTapped:(id)sender {
    if ([self.delegate respondsToSelector:@selector(addButtonView:presentAlertController:)]) {
        ...
        UIAlertAction *scanReceipt = [UIAlertAction
                                         actionWithTitle:@"Scan Receipt"
                                         style:UIAlertActionStyleDefault
                                         handler:^(UIAlertAction * action) {
                                             [alert dismissViewControllerAnimated:YES completion:nil];
                                             if ([self.delegate respondsToSelector:@selector(addButtonView:alertControllerForScanReceipt:)]) {
                                                 [self.delegate addButtonView:self alertControllerForScanReceipt:alert];
                                             }
                                         }];
